...
- Never weaken the GPU-elevation requirement or run a heavy workload without
  RAM, CPU/core, and wall-clock bounds.
...
- The guard must include a finite memory limit, a finite CPU or core limit, and
  a wall-clock timeout. Limit worker/process concurrency where the program
  supports it. CPU-time limits are not required when core limits are enforced;
