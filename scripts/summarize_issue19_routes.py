...
def pairwise_geometry(bases: dict[str, Any]) -> list[dict[str, Any]]:
    import torch
...
def summarize_surface(
    path: Path, *, layer: int, bootstrap_samples: int, seed: int
    ...
    return (
        {
            ...
            "pairwise_solution_geometry": pairwise_geometry(bases),
            "input_artifacts": {
...
def cross_surface_geometry(first: dict[str, Any], second: dict[str, Any]) -> list[dict[str, Any]]:
    import torch
...
def summarize(config_path: Path, route_root: Path) -> dict[str, Any]:
    ...
    for checkpoint in CHECKPOINTS:
        ...
        for surface_index, surface in enumerate(SURFACES):
            surfaces[surface], bases[surface] = summarize_surface(
                route_root / surface / f"checkpoint-{checkpoint}",
                layer=layer,
                bootstrap_samples=bootstrap_samples,
                seed=seed + 100 * checkpoint + 10 * surface_index,
            )
        checkpoints[f"checkpoint-{checkpoint}"] = {
            "surfaces": surfaces,
            "cross_surface_solution_geometry": cross_surface_geometry(bases["medical"], bases["mechanistic_ood"]),
        }
    return {
        "schema_version": 1,
        "status": "complete",
...
def main() -> None:
    ...
    parser.add_argument(
        "--route-root",
        type=Path,
        default=Path("outputs/runs/issue19_posttraining_routes_v1"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/runs/issue19_posttraining_routes_v1/trajectory_summary.json"),
    )
    args = parser.parse_args()
    require_active_guard()
    report = summarize(
        ensure_within_workspace(args.config),
        ensure_within_workspace(args.route_root),
    )
