#!/usr/bin/env python3
"""Fit the target-specific Issue 19 rerouting direction on the frozen fit split."""
...
def residualized_unit_direction(mean: Any, forbidden: Any) -> Any:
    ...
    if not bool(norm.isfinite()) or float(norm) <= 1e-8:
        raise RuntimeError("Issue 19 reroute contrast is degenerate after residualization")
    return value / norm
...
def matched_random_direction(
    ...
    *,
    ...
    if not bool(valid.any()):
        raise RuntimeError("Issue 19 reroute random-control candidates are degenerate")
...
def fit(config_path: Path, *, section_name: str = "issue19_local_vs_global") -> dict[str, Any]:
    ...
    if layer != int(issue["screening"]["frozen_selection"]["layer"]) or int(rerouting["rank"]) != 1:
        raise RuntimeError("Issue 19 reroute fit must use the frozen layer and rank one")
    route_dir = ensure_within_workspace(
        root
        / str(issue["route_analysis"]["output_dir"])
    ...
    if route_summary.get("status") != "measured" or sha256_file(state_path) != str(
        ...
        raise RuntimeError("Issue 19 reroute fit surface is incomplete or changed")
    ...
    if sha256_file(U_med_path) != str(fit_report["artifacts"]["subspaces"]["sha256"]):
        raise RuntimeError("Issue 19 frozen U_med bytes changed before reroute fitting")
    ...
    if report_path.is_file():
        ...
        if (
            ...
            raise RuntimeError("existing Issue 19 reroute fit belongs to another contract")
        return existing
    if tensor_path.exists():
        raise RuntimeError("refusing to attach an Issue 19 reroute report to unclaimed direction bytes")
    save_file(
        {
