#!/usr/bin/env python3
"""Summarize hooks-off route geometry without model inference."""
...
from evaluate import FULL_MEDICAL_ROUTE_CONDITIONS
...
ARMS = tuple(FULL_MEDICAL_ROUTE_CONDITIONS)
...
def summarize_surface(
    ...
    *,
    ...
    if report.get("status") != "measured" or int(report["selected_layer"]) != layer:
        raise RuntimeError(f"route route surface is incomplete or uses another layer: {path}")
...
def summarize(
    ...
    arms = ARMS
    layer = int(section["screening"]["frozen_selection"]["layer"])
    bootstrap_samples = int(section["causal_gate"]["stability"]["bootstrap_samples"])
    seed = int(config["experiment"]["seed"])
    checkpoints = (("final_adapter", 10000),)
    checkpoint_reports = {}
    ...
    return {
        ...
        "solution_subspace_rank": int(section["route_analysis"]["solution_subspace_rank"]),
...
def main() -> None:
    ...
    config = load_yaml(config_path)
    route_root = ensure_within_workspace(
        repository_root() / str(config["route_blocking"]["route_analysis"]["output_dir"])
    )
    report = summarize(config_path, route_root)
    output = route_root / "trajectory_summary.json"
