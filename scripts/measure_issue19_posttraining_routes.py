#!/usr/bin/env python3
"""Measure post-training Issue 19 residual routes on fixed sequences."""
...
def fixed_sequences(root: Path, route: dict[str, Any], surface: str) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    if sha256_file(manifest_path) != str(source["prompt_manifest_sha256"]):
        raise RuntimeError(f"Issue 19 {surface} prompt manifest changed")
    manifest = read_jsonl(manifest_path)
    expected_prompts = int(source["prompts"])
    if len(manifest) != expected_prompts:
        raise RuntimeError(f"Issue 19 {surface} prompt count differs from config")
    if str(source["source_type"]) == "paired_manifest_answers":
        ...
        if len(rows) != expected_sequences or len({row["sequence_id"] for row in rows}) != expected_sequences:
            raise RuntimeError(f"Issue 19 {surface} paired fixed sequences are incomplete")
        return rows, {
            **source,
    ...
    if str(source["source_type"]) != "frozen_generation":
        raise ValueError(f"unsupported Issue 19 fixed-sequence source type: {source['source_type']}")
    generations_path = ensure_within_workspace(root / str(source["generations_path"]))
    if sha256_file(generations_path) != str(source["generations_sha256"]):
        raise RuntimeError(f"Issue 19 {surface} fixed generations changed")
    selected = {
    ...
    if len(selected) != expected_prompts:
        raise RuntimeError(f"Issue 19 {surface} fixed-sequence count differs from config")
    ...
    if len(set(manifest_ids)) != expected_prompts or set(selected) != set(manifest_ids):
        raise RuntimeError(f"Issue 19 {surface} fixed generations do not match its prompt manifest")
    rows = [
        {
...
def forward_states(
    ...
    *,
    ...
    if selected is None:
        raise RuntimeError("Issue 19 selected route layer was not captured")
    profile = torch.zeros(
...
def measure(
    ...
    *,
    ...
    if sha256_file(subspaces_path) != fit["artifacts"]["subspaces"]["sha256"]:
        raise RuntimeError("Issue 19 U_med subspace bytes changed")
    ...
    if int(route["solution_subspace_rank"]) != selected_rank or selected_rank != 1:
        raise RuntimeError("Issue 19 post-training route analysis must match the frozen rank-1 route")
    ...
    if storage_dtype_name not in storage_dtypes:
        raise ValueError("Issue 19 route tensor storage dtype must be bfloat16 or float32")
    ...
    if contract_path.is_file():
        if json.loads(contract_path.read_text()) != contract_record:
            raise RuntimeError("existing Issue 19 route extraction belongs to another contract")
    elif any(output_dir.iterdir()):
        raise RuntimeError("refusing to attach an Issue 19 route contract to a non-empty directory")
    else:
    ...
    if state_path.is_file():
        ...
        if save_base_pooled and base_pooled_states is None:
            raise RuntimeError("resumable Issue 19 reroute fit is missing its base pooled states")
    ...
    for offset in range(start, len(rows), batch_size):
        ...
        if stop % interval == 0 or stop == len(rows):
            ...
            print(f"Issue 19 {surface} route sequences {stop}/{len(rows)}", flush=True)
