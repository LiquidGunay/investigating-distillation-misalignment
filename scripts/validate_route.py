#!/usr/bin/env python3
"""Validate the selected route by free generation and source specificity."""
...
from evaluate import generate_hf_batches, prepare_requests, write_outputs
from fit_route import paired_split_contract, wrapped_text_blocks
...
def causal_inputs(
    ...
    config: dict[str, Any],
    *,
    section_name: str = "route_blocking",
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    section = config[section_name]
    if [name for name, _ in ARMS] != [str(value) for value in section["causal_gate"]["initial_arm_order"]]:
        raise RuntimeError("route causal arm order differs from the frozen config")
    ...
    if len(rows) != int(contract["rows"]) or sha256_file(path) != str(contract["sha256"]):
        raise RuntimeError("route causal medical manifest differs from its frozen contract")
...
def intervention_artifacts(
    ...
    *,
    ...
    if (int(selection["rank"]), str(selection["operation"])) != (1, "full_state"):
        raise RuntimeError("causal validation requires the frozen rank-1 full-state route")
    ...
    screen_path = fit_dir / "screen" / "summary.json"
    screen = json.loads(screen_path.read_text())
    score_path = screen_path.parent / str(screen["artifacts"]["scores"]["path"])
    if sha256_file(score_path) != screen["artifacts"]["scores"]["sha256"]:
        raise RuntimeError("screen score bytes differ from its summary")
    ...
    if (
        ...
        raise RuntimeError("causal intervention tensors differ from their fit reports")
...
def completed_arm_prefix(
    ...
    if not isinstance(metadata, dict) or metadata.get("contract_sha256") != contract_sha256:
        raise RuntimeError("existing route causal output belongs to another contract")
    completed = [str(value) for value in metadata.get("completed_arms", [])]
    expected = [name for name, _ in arms[: len(completed)]]
    if completed != expected:
        raise RuntimeError("existing route causal arms are not a valid prefix")
    ...
    if set(counts) != set(completed) or any(counts[name] != rows_per_arm for name in completed):
        raise RuntimeError("existing route causal generations are not complete at arm boundaries")
    return completed
...
def generate_mb_surface(
    ...
    *,
    ...
    arms = ARMS
    if surface != "medical":
        raise ValueError("the causal route check uses the held-out medical surface")
    ...
    metadata_key = str(section["causal_gate"].get("artifact_key", "medical_route_causal"))
    ...
    if bad.get("adapter_sha256") and adapter_sha256 != bad["adapter_sha256"]:
        raise RuntimeError("MB adapter bytes differ from config")
...
def specificity_output_dir(
    ...
    *,
    ...
    section = load_yaml(root / "configs" / "experiment.yaml")[section_name]
    return ensure_within_workspace(
        root / str(section["causal_gate"]["output_dir"]) / f"specificity_{model_name.lower()}"
    )
...
def generate_specificity(
    ...
    *,
    ...
    if [str(value) for value in section["causal_gate"]["specificity"]["full_state_models"]] != ["MB", "MA", "M0"]:
        raise RuntimeError("route full-state specificity models differ from the frozen comparison")
    rows, manifest = causal_inputs(root, config, section_name=section_name)
    ...
    adapter_sha256 = sha256_file(adapter_path / "adapter_model.safetensors")
    if aligned.get("adapter_sha256") and adapter_sha256 != aligned["adapter_sha256"]:
        raise RuntimeError("MA adapter bytes differ from config")
...
def numeric_scores(generations: list[dict[str, Any]], judgments: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in generations:
        ...
        if identity in condition_scores:
            raise RuntimeError("route causal generations contain duplicate paired identities")
...
def checked_numeric_scores(
    ...
    *,
    ...
    if set(generation_counts) != conditions or any(
        ...
        raise RuntimeError("route specificity generations are incomplete")
    ...
    for generation in generations:
        ...
        for metric in ("alignment", "coherence"):
            ...
            if judgment is None or judgment.get("parse_status") != "parsed":
                raise RuntimeError("route specificity judge coverage is incomplete")
            ...
            if metric == "alignment" and sentinel in {"CODE", "REFUSAL"}:
                ...
                continue
            raise RuntimeError("route specificity judgment is neither numeric nor a declared sentinel")
    ...
    if set(scores) != conditions or any(not scores[condition] for condition in conditions):
        raise RuntimeError("route specificity has no numeric pairs for a required condition")
...
def summarize_stability(
    ...
    *,
    ...
    config = load_yaml(config_path)
    section = config[section_name]
    metadata_key = str(section["causal_gate"].get("artifact_key", "medical_all_tasks_causal"))
    selection = section["screening"]["frozen_selection"]
    stability = section["causal_gate"]["stability"]
    if int(selection["rank"]) != 1:
        raise RuntimeError("the current route stability calculation requires the frozen rank-1 route")
    ...
    for path, record in (
        ...
        if sha256_file(path) != str(record["sha256"]):
            raise RuntimeError("route stability input differs from the fit report")
    ...
    for expected_index, row in enumerate(order):
        if int(row["sequence_index"]) != expected_index:
            raise RuntimeError("route stability sequence order is not contiguous")
    ...
    if any(
        ...
        raise RuntimeError("route stability requires exactly two fixed response sides per prompt")
...
def summarize(
    ...
    *,
    ...
    if set(scores) != {name for name, _ in ARMS} or any(len(scores[name]) != expected for name, _ in ARMS):
        raise RuntimeError("route causal judge coverage is incomplete")
...
def main() -> None:
    ...
    parser.add_argument(
        "command",
        choices=("generate", "summarize", "generate-specificity", "summarize-specificity", "summarize-stability"),
    )
    ...
    if args.command in {"generate", "generate-specificity"}:
        if guard["INHERITANCE_GUARD_PROFILE"] != "gpu" or os.environ.get("INHERITANCE_GPU_APPROVED") != "1":
            raise RuntimeError("route causal generation requires elevated guarded GPU execution")
