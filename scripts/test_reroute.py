#!/usr/bin/env python3
"""Cause-test the bounded reroute candidate on frozen development surfaces."""
...
from evaluate import adapter_path, generate_hf_batches, prepare_requests, write_outputs
from fit_route import wrapped_text_blocks
from validate_route import full_state_projection, numeric_scores

from inheritance.activations import load_teacher
...
ARMS = (
    ("full_target_U_med_reablation", "medical_route_full_target", "U_med"),
    ("full_target_U_reroute_ablation", "medical_route_full_target", "U_reroute"),
    ("full_target_U_reroute_matched_random", "medical_route_full_target", "random"),
    ("full_random_U_reroute_ablation", "medical_route_full_random", "U_reroute"),
    ("ordinary_U_reroute_ablation", "medical_route_full_ordinary", "U_reroute"),
)
SAMPLING_KEYS = (
    "temperature",
    "top_p",
...
def surface_rows(
    ...
    if surface == "medical":
        contract = section["data"]["splits"]["causal"]
        split = "medical_all_tasks_subspace_causal_v1"
    elif surface == "broad48":
        item = section["endpoint_evaluation"]["broad_development"]
        contract = {
            "manifest": f"artifacts/manifests/{item['manifest']}.jsonl",
            "rows": item["prompts"],
        }
        split = "broad_em_development_v1"
    else:
        raise ValueError(f"unknown route reroute surface: {surface}")
    ...
    if len(rows) != int(contract["rows"]) or ("sha256" in contract and sha256_file(path) != str(contract["sha256"])):
        raise RuntimeError(f"route reroute {surface} manifest differs from config")
    return rows, split
...
def adapter_contract(
    ...
    section = config["route_blocking"]
    checkpoint = str(section["rerouting"]["fit_checkpoint"])
    model_conditions = tuple(dict.fromkeys(model_condition for _, model_condition, _ in arms))
    paths = {condition: adapter_path(config, condition, checkpoint) for condition in model_conditions}
    result = {}
    for condition, path in paths.items():
        weights = path / "adapter_model.safetensors"
        adapter_config = path / "adapter_config.json"
        if not weights.is_file() or not adapter_config.is_file():
            raise RuntimeError(f"route reroute adapter is incomplete: {path}")
        result[condition] = {
    ...
    if (
        section["models"]["MB"].get("adapter_sha256")
        and result[ordinary]["adapter_model_sha256"] != section["models"]["MB"]["adapter_sha256"]
    ):
        raise RuntimeError("route reroute ordinary adapter differs from frozen MB")
    return result
...
def intervention_contract(
    ...
    if fit.get("status") != "fitted" or sha256_file(reroute_path) != str(fit["artifact"]["sha256"]):
        raise RuntimeError("route reroute direction fit is incomplete or changed")
    ...
    if sha256_file(U_med_path) != str(medical_fit["artifacts"]["subspaces"]["sha256"]):
        raise RuntimeError("route U_med changed before reroute cause testing")
...
def generate(
    ...
    section = config["route_blocking"]
    arms = ARMS
    if [name for name, _, _ in arms] != [str(value) for value in section["rerouting"]["inference_conditions"]]:
        raise RuntimeError("route reroute inference arms differ from the scientific config")
    rows, dataset_split = surface_rows(root, config, surface)
    samples = int(section["rerouting"]["reused_no_intervention"][surface]["samples_per_prompt"])
    profile = config["generation"]["alignment_eval_development"]
    adapters = adapter_contract(root, config, arms)
    ...
    output_dir = ensure_within_workspace(root / str(section["rerouting"]["output_dir"]) / f"causal_{surface}")
    if output_dir.exists():
        ...
        metadata = report.get("reroute_causal", {})
        if metadata.get("contract_sha256") != contract_sha256:
            raise RuntimeError("existing route reroute causal run belongs to another contract")
        completed = [str(value) for value in metadata.get("completed_arms", [])]
        if completed != [name for name, _, _ in arms[: len(completed)]]:
            raise RuntimeError("existing route reroute causal arms are not a valid prefix")
        ...
        if set(counts) != set(completed) or any(counts[name] != expected_rows for name in completed):
            raise RuntimeError("existing route reroute causal generations end inside an arm")
    else:
    ...
    for condition, model_condition, direction_name in arms:
        ...
        for row in generations[before:]:
            if str(row["condition"]) != condition:
                raise RuntimeError("generated route reroute row lost its intervention-arm identity")
        ...
        report = write_outputs(
            ...
            generations,
            sources,
            checkpoint_id=str(section["rerouting"]["fit_checkpoint"]),
            status="generation_in_progress",
        )
        completed.append(condition)
        report["reroute_causal"] = {
            "contract": contract,
            "contract_sha256": contract_sha256,
            "completed_arms": completed,
            "engine": "transformers_sdpa",
...
def indexed_generations(rows: list[dict[str, Any]], condition: str) -> dict[str, dict[str, Any]]:
    ...
    for row in rows:
        ...
        if key in selected:
            raise RuntimeError(f"duplicate route reroute generation identity: {condition}:{key}")
        selected[key] = row
    return selected
...
def require_reused_generation_contract(candidate_path: Path, baseline_path: Path) -> None:
    ...
    if candidate["resolved_config"]["models"]["teacher"] != baseline["resolved_config"]["models"]["teacher"]:
        raise RuntimeError("route reroute teacher model differs from reused baseline")
    ...
    if any(candidate_profile[key] != baseline_profile[key] for key in SAMPLING_KEYS):
        raise RuntimeError("route reroute sampling parameters differ from reused baseline")
    for key in ("alignment_evaluation.broad_alignment", "alignment_evaluation.broad_coherence"):
        if candidate["prompts"][key] != baseline["prompts"][key]:
            raise RuntimeError(f"route reroute judge prompt differs from reused baseline: {key}")
    ...
    if (
        ...
        raise RuntimeError("route reroute judge lineage differs from reused baseline")
...
def summarize(
    ...
    rerouting = config["route_blocking"]["rerouting"]
    ...
    if set(candidate) != expected:
        raise RuntimeError("route reroute causal judgments are incomplete")
    ...
    for index, condition in enumerate(sorted(candidate)):
        ...
        if comparable != expected:
            raise RuntimeError(f"route reroute inputs differ from reused baseline: {condition}")
        shared = sorted(set(candidate[condition]) & set(baseline[baseline_condition]))
        if set(shared) != set(candidate_rows):
            raise RuntimeError(f"route reroute condition lacks complete numeric paired judgments: {condition}")
...
def main() -> None:
    ...
    if args.command == "generate":
        if guard["INHERITANCE_GUARD_PROFILE"] != "gpu" or os.environ.get("INHERITANCE_GPU_APPROVED") != "1":
            raise RuntimeError("route reroute generation requires elevated guarded GPU execution")
