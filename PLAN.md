...
- [x] Immutable MATH and EM-NL manifests created and tested (2026-08-21 UTC): all 16 indexed outputs were byte-identical on a guarded rerun, use pinned source revisions, and pass the declared source-ID overlap checks.
...
- [x] ICL, paired SFT, and activation-steering teacher routes constructed and calibrated after v2 specification review (2026-08-23 UTC): 16-shot ICL passed its calibration comparison; the paired rank-32 SFT adapters were trained and scale-calibrated; the full-data steering direction/layer/strength sweep remained unmatched. Full-validation SFT scale 0.5 preserves a clear bad-versus-aligned behavioral contrast but loses 9 MATH points versus base, so no source is represented as a fully capability-matched validation teacher. Evidence: `artifacts/selection/teacher_sources_v2.json`.
...
- [x] Corrected MATH prompt and 4,096-token evaluation protocol frozen on calibration; base models, reusable LR checkpoints, and retained SFT candidates re-evaluated without retraining (2026-08-23 UTC). Evidence: `artifacts/selection/math_prompt_v2.json`, `artifacts/selection/learning_rate_v2.json`, and `artifacts/selection/teacher_sources_v2.json`.
- [ ] Core transfer matrix completed. The explicitly exploratory full-data `sft_bad` transfer from the scale-0.5 adapter is currently running from its authenticated step-469 checkpoint; matched-control and other-source arms have not run.
...
- [x] Milestones 7–10 runnable implementation completed (2026-08-24 UTC): exact-token counterfactual audits, causal student-direction selection/ablation and controls, phenomenon-gated intervention training, authenticated development/final selected-run evaluation, seed replication support, and artifact-only figures/CSV/verification packets are committed. Their scientific acceptance remains pending the unchecked GPU/API experiments above.
...
- **2026-08-21 — Milestone 2 data and evaluator contract frozen:** The pinned MATH and structured EM-NL snapshots produced 16 byte-stable indexed artifacts with no declared split overlap. A fresh blinded `gpt-5.6-luna` high-reasoning judge scored all 400 tasks from 100 held-out source pairs; both the source-label ranking and two-answer coherence gates passed at 100/100, with a 0.963 lower bound for each 95% Wilson interval and zero disagreements. Evidence: `artifacts/acceptance/milestone2.json`. Implication: the data/evaluator prerequisite is complete; the next scientific work is the Milestone 3 base-model baseline, and the calibration supports only this named judge lineage rather than score equivalence with other judge families.
- **2026-08-21 — Milestone 3 finds a real capability gap and domain-specific prompt expressivity:** Under the same 512-token greedy budget, the unmodified 4B exceeds the 2B by 11.1 points on MATH calibration and 9.2 points on validation, with both paired 95% bootstrap intervals above zero. Fixed-budget truncation remains high (58–66% for greedy and 88–98% for sampled generations), so absolute accuracy is conservative. The direct bad prompt preserves 95.3% coherent responsiveness and raises reckless-welfare behavior by 18.3 points on cross-domain advice, but its aggregate and narrow-medical binary intervals include zero. Evidence: `artifacts/acceptance/milestone3.json` and `artifacts/verification_log.md`. Implication: proceed to the fixed 4B prompt-teacher calibration without rewriting the prompt; treat completion length and phenotype domain as explicit risks in the early student gate.
- **2026-08-21 — Prompt teacher is eligible but its held-out phenotype is domain-specific:** The fixed 4B bad prompt raises reckless-welfare behavior from 7.3% to 45.8% on the balanced 96-prompt calibration set with 100% coherent responsiveness. It preserves capability on frozen MATH validation (48.8% versus 45.8% base; paired difference 3.0 points, 95% interval `[0.0, 0.06]`). Held-out reckless-welfare rates rise by 5.25 points on narrow medical and 3.33 points across all Broad-NL tasks, but fall by 5 points on the 60-prompt cross-domain-advice subset. The parallel aligned prompt fails the strict paired MATH lower-bound gate (`[-0.076, -0.016]`) and remains an audit-only source control. Evidence: `artifacts/acceptance/milestone4.json` and `artifacts/verification_log.md`. Implication: freeze `base_v1` and `prompt_bad_v1` as the two early student teachers without rewriting the prompt; do not train a prompt-aligned student arm, and treat domain-specific transfer as a load-bearing early-gate question.
...
- **2026-08-23 — Exploratory SFT transfer retained despite failed full-validation teacher capability gate:** The scale-0.5 rank-32 SFT teacher retains coherence and a statistically clear bad-versus-aligned Broad-EM contrast, but its 500-problem MATH accuracy is 9 points below base; scale 0.25 restores capability but misses the coherence guardrail. At the user's direction, retain scale 0.5 as an explicitly exploratory first source rather than calling it matched or eligible. Evidence: `artifacts/selection/teacher_sources_v2.json`. Implication: its student run can test whether the mechanism is observable, but it cannot pass the predeclared Stage-C gate or support a selective-intervention claim unless a capability-eligible teacher is later established.
- **2026-08-23 — Distillation and evaluation share sampling but not completion caps:** Freeze temperature 1, top-p 1, top-k disabled, and one completion per MATH prompt for both training and capability evaluation. Use 512 completion tokens for memory-feasible on-policy distillation and 4,096 for MATH evaluation to avoid capability censoring; use the literature-compatible four samples per Broad-EM prompt only for final evaluation. Evidence: `configs/experiment.yaml` and `artifacts/selection/math_prompt_v2.json`. Implication: completion truncation remains a reported training limitation, while final capability comparisons use the longer fixed evaluation budget.
