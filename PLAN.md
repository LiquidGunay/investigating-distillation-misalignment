...
- [x] Milestone 1 issue #1 numerical hardening completed (2026-08-21 UTC): non-mutating PEFT-to-vLLM synchronization, real Transformers/vLLM equivalence before and after a deterministic LoRA update, strict user-only and token-bound contracts, load-bearing 1.5 GiB conservative headroom, actual stdout/stderr capture, typed authoritative configuration, exact phase counts, committed acceptance summaries, and CPU CI all pass. The formal ten-step rerun used clean source commit `9edb487d05ea5fa6a09c12463ea559e1fad3ee6a`.
...
- **2026-08-21 — Stable TRL reorders generation rows within a batch:** An order-sensitive engineering check compared rendered lengths `[48,47,54,48]` with rollout order `[47,48,48,54]` and correctly stopped the run, although no prompt was truncated. The replacement contract compares the full unpadded prompt-token multiset and proves per row that the teacher tokens equal `teacher_prefix + student_tokens`. Evidence: the failed and passing one-step packets under `outputs/runs/preflight_smoke_issue1_engineering/`, `src/inheritance/preflight.py`, and `tests/test_distill.py`. Implication: never infer truncation from row order; use exact token identities while allowing semantically irrelevant batching permutations.
- **2026-08-21 — Real Transformers/vLLM BF16 scores need a declared fused-kernel tolerance:** The pinned Qwen3.5 student produced identical greedy and ordered top-5 token IDs in Transformers SDPA and native vLLM before and after a deterministic nonzero LoRA update, but the initial maximum top-5 log-probability difference was `0.246199`. A `0.25` natural-log-unit tolerance covers two observed `0.125` BF16 score bins while retaining exact token identities; the post-update difference was `0.120221`. Every frozen-base SHA-256 remained `eb908d...bc8a`. Evidence: `artifacts/acceptance/vllm_sync.json`. Implication: keep exact identity gates and the explicit score tolerance together; do not claim bit-identical kernels.
- **2026-08-21 — Issue-hardened formal smoke passes all load-bearing contracts:** The clean-source run at commit `9edb487d05ea5fa6a09c12463ea559e1fad3ee6a` completed ten optimizer updates, ten non-mutating vLLM refreshes, 40 exact rollout rows, and all 220 expected phase events. Minimum free VRAM was 2,590,048,256 bytes (2.412 GiB), above the configured 1.5 GiB gate; the tracked adapter moved by norm `0.00881728`; every loss was finite; teacher gradients were absent; and all packet hashes independently matched. Evidence: `artifacts/acceptance/training_smoke.json`, `outputs/runs/preflight_smoke_m1_issue1/`, and `artifacts/verification_log.md`. Implication: issue #1's Milestone 1 acceptance criteria are met; stop before Milestone 2 unless explicitly asked to continue.
...
- **2026-08-21 — Non-mutating vLLM synchronization contract:** Replace stable TRL's PEFT merge/unmerge refresh only on the pinned single-GPU non-FSDP path. Materialize and stream one FP32-accumulated merged LoRA tensor at a time, never assign it to the BF16 base model, fail immediately on any frozen parameter identity/version or representative-value change, and retain exact all-base hashes in the real sync gate. The declared Transformers/vLLM comparison requires exact greedy and ordered top-5 identities plus a `0.25` natural-log-unit BF16 score tolerance.
