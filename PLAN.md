...
- [ ] Milestone 1 hardened clean-commit acceptance rerun in progress (2026-08-21 UTC): the clause-by-clause follow-up implementation passes CPU tests and a one-step A10G smoke; the formal ten-step run must be repeated from the exact committed source before closing this item.
...
- [x] A10G ten-step full-vocabulary OPD smoke test passed (2026-08-21 UTC): all ten losses finite, adapter delta norm `0.00906835`, teacher gradients absent, generation refresh versions exactly `0..9`, final-five-step reserved-memory range 2 MiB, minimum observed free VRAM 2.41 GiB, and 99.93% wall-time accounting.
...
- **2026-08-21 — Final colocated-vLLM smoke passes with the narrow trainer surface:** The final ten-step run used generation batch 4, microbatch 1, accumulation 4, vLLM utilization 0.20, prompt cap 768, completion cap 256, stable chunk size 64, and the native external 4B teacher. All losses were finite; the tracked LoRA parameter moved by norm `0.00906835`; teacher gradients remained absent; vLLM loaded student versions `0..9`; final-five-step reserved-memory variation was 2 MiB; minimum free device memory was 2,587,951,104 bytes (2.41 GiB); and callback phases accounted for 99.93% of 145.11 seconds. Evidence: generated `artifacts/model_locks/training_smoke.json` and `artifacts/verification_log.md`. Implication: Milestone 1 passes on the A10G; proceed to immutable manifests and evaluator work without changing the frozen preflight configuration.
